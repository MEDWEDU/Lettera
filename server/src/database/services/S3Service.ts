...
class S3Service {
  ...
  async uploadFile(
    ...
    try {
      ...
      if (!this.validateFileSize(buffer.length, fileType)) {
        throw new Error(`Размер файла превышает лимит для ${fileType} файлов`);
      }
  ...
  async deleteFile(url: string, userId?: string): Promise<void> {
    try {
      ...
      throw new Error(`Ошибка удаления файла: ${error instanceof Error ? error.message : 'Неизвестная ошибка'}`);
    }
  }
  ...
  async generatePresignedUrl(key: string, expiresIn: number = 3600): Promise<string> {
    try {
      ...
      throw new Error(`Ошибка генерации временного URL: ${error instanceof Error ? error.message : 'Неизвестная ошибка'}`);
    }
  }
